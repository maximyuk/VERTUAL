...
try:
  ...
  try:
    cursor.execute(
      "CREATE DATABASE IF NOT EXISTS virtual")  #тіп створює бд
  finally:
    connect.commit()
...
#----------------------------------------------------------создає таблицю узерс коли бот добавляється на сервер----------------------------------------------------------------------------------------------------
    cursor.execute(f"create table if not exists virtual.users(guild_name varchar(50), user_name varchar(50),guild int default null, user_id int default null, mute int default null, ban int default null, warn int default null, admin int default null)")  # создає таблицю
    connect.commit()
 
    for guild in bot.guilds:
        for member in guild.members:
            cursor.execute(f"select * from virtual.users where guild = {guild.id} and user_id = {member.id}")
            if cursor.fetchone() is None:
                cursor.execute(f"insert into virtual.users values('{member.guild.name}', '{member}',{member.guild.id}, {member.id}, 0, 0, 0, 0)")
                connect.commit()
            else:
                print('\u200b')

    cursor.execute(f"UPDATE virtual.users SET admin = 6 WHERE guild = {guild.id} AND user_id = {guild.owner.id}")
    connect.commit()


#--------------------------------------------------------------создає таблицю guilds коли бот добавляється на сервер-----------------------------------------------------------------------
    cursor.execute(f"create table if not exists virtual.guilds(guild_name varchar(50) , guild_id bigint default null, welcome text, cmdkick int default 3, cmdwelcome int default 5)")  # создає таблицю
    connect.commit()
 
    for guild in bot.guilds:
        for member in guild.members:
            cursor.execute(f"select * from virtual.guilds where guild_name = '{guild.name}' and guild_id = {guild.id}")
            if cursor.fetchone() is None:
                cursor.execute(f"insert into virtual.guilds values('{member.guild.name}', {member.guild.id}, NOT NULL, NOT NULL, NOT NULL)")
                connect.commit()
            else:
                print('\u200b')
...
async def on_ready():
 ...
 #-------------------------------------------------------------------создає таблицю users коли бот включається-------------------------------------------------------------------
    cursor.execute(f"create table if not exists virtual.users(guild_name varchar(33), user_name varchar(33),guild bigint default null, user_id bigint default null, mute int default null, ban int default null, warn int default null, admin int default null)")  # создає таблицю
    connect.commit()
...
#-------------------------------------------------------------Создає таблицю guilds коли бот включається-------------------------------------------------------------------
    ...
    cursor.execute(f"create table if not exists virtual.guilds(guild_name varchar(50), guild_id bigint default null, welcome text default null, cmdkick int default 3, cmdwelcome int default 5)")  # создає таблицю
    connect.commit()
 
    for guild in bot.guilds:
        cursor.execute(f"select * from virtual.guilds where guild_name = '{guild.name}' and guild_id = {guild.id}")
        if cursor.fetchone() is None:
            cursor.execute(f"insert into virtual.guilds(guild_name, guild_id) values('{guild.name}' , {guild.id})")
            connect.commit()
        else:
            print('\u200b')
...
async def on_member_join(member):
    ...
    for member in member.guild.members:
        cursor.execute(f"select guild, user_id from virtual.users where guild = {member.guild.id} and user_id = {member.id}")
        if cursor.fetchone() is None:
            cursor.execute(f"insert into virtual.users values('{member.guild.name}', '{member}',{member.guild.id}, {member.id}, 0, 0, 0, 0)")
            connect.commit()
        else:
...
#-------------------------------------------------------------------Добававляє всю інфу в guilds коли чел заходить на сервер----------------------------------------------
    for member in member.guild.members:
        cursor.execute(f"select guild_name ,guild_id from virtual.guilds where guild_name = '{member.guild.name}' and guild_id = {member.guild.id}")
        if cursor.fetchone() is None:
            cursor.execute(
                f"insert into virtual.guilds values({member.guild.id} ,{member.guild.id}, NOT NULL, NOT NULL, NOT NULL)")
            connect.commit()
        else:
            print('\u200b')
...
async def on_member_remove(member):
    ...
    cursor.execute(
        f"update virtual.users set admin = 0 where user_id = {member.id} and guild = {member.guild.id}")
    connect.commit()
...
async def setadm(ctx, user:disnake.Member,* , lvl:int): 
    cursor.execute(f"select * from virtual.users where guild = '{ctx.guild.id}' and user_id = '{ctx.author.id}'")
    _admin = cursor.fetchone()['admin']
    cursor.execute(f"select * from virtual.users where guild = '{ctx.guild.id}' and user_id = '{user.id}'")
        ...
        if _admin < admins:
            ...
            if user == ctx.author:
                ...
                if lvl >5:
                    ...
                    if _admin == 5:
                        if lvl == 0:
                            cursor.execute(f"update virtual.users set admin = 0 where user_id = '{user.id}' and guild = '{ctx.guild.id}'")
                            connect.commit()
                        ...
                        if lvl == 1:
                            cursor.execute(f"update virtual.users set admin = 1 where user_id = '{user.id}' and guild = '{ctx.guild.id}'")
                            connect.commit()
                        ...
                        if lvl == 2:
                            cursor.execute(f"update virtual.users set admin = 2 where user_id = '{user.id}' and guild = '{ctx.guild.id}'")
                            connect.commit()
                            embed=disnake.Embed(title=f'Вам выдали `{lvl}` уровень администратора на сервере `{ctx.guild.name}`', description=f'Команду выполнил: {ctx.author.mention}', color= 0x00fa6a)
                            await user.send(embed=embed)
                        ...
                        if lvl == 3:
                            cursor.execute(f"update virtual.users set admin = 3 where user_id = '{user.id}' and guild = '{ctx.guild.id}'")
                            connect.commit()
                            embed=disnake.Embed(title=f'Вам выдали `{lvl}` уровень администратора на сервере `{ctx.guild.name}`', description=f'Команду выполнил: {ctx.author.mention}', color= 0x00fa6a)
                            await user.send(embed=embed)
                        ...
                        if lvl == 4:
                            cursor.execute(f"update virtual.users set admin = 4 where user_id = '{user.id}' and guild = '{ctx.guild.id}'")
                            connect.commit()
                            embed=disnake.Embed(title=f'Вам выдали `{lvl}` уровень администратора на сервере `{ctx.guild.name}`', description=f'Команду выполнил: {ctx.author.mention}', color= 0x00fa6a)
                            await user.send(embed=embed)
                    ...
                    if _admin == 6:
                        ...
                        if lvl == 4:
                            ...
                            await ctx.send(embed=embed1)
                        if lvl == 5:
                            cursor.execute(f"update virtual.users set admin = 5 where user_id = '{user.id}' and guild = '{ctx.guild.id}'")
                            connect.commit()
                            embed=disnake.Embed(title=f'Вам выдали `{lvl}` уровень администратора на сервере `{ctx.guild.name}`', description=f'Команду выполнил: {ctx.author.mention}', color= 0x00fa6a)
                            await user.send(embed=embed)
...
async def mute(ctx, user: disnake.Member,  time:int = None,*, reason=None):
    try:
        cursor.execute(f"select * from virtual.users where guild = '{ctx.guild.id}' and user_id = '{ctx.message.author.id}'")
        _admin = cursor.fetchone()['admin']
        cursor.execute(f"select * from virtual.users where guild = '{ctx.guild.id}' and user_id = '{user.id}'")
        admins = cursor.fetchone()['admin']
        if _admin <3:
